...
const DEMO_QUERIES = [
  { label: 'Find Glasses', query: 'Where are my glasses?' },
  { label: 'Find Keys', query: 'Where are my keys?' },
  { label: 'Who Is This?', query: 'Who is this person?' },
  { label: 'Medication', query: 'Did I take my medicine?' },
  { label: 'Stove Safe?', query: 'Is the stove on?' },
];
...
function getResponse(query) {
  ...
  if (q.includes('glasses')) return RESPONSES.glasses;
  if (q.includes('keys')) return RESPONSES.keys;
  if (q.includes('phone')) return RESPONSES.phone;
  if (q.includes('who')) return RESPONSES.who;
  if (q.includes('medicine') || q.includes('pill') || q.includes('medication')) return RESPONSES.medicine;
  if (q.includes('stove') || q.includes('oven')) return RESPONSES.stove;
      ...
      <View style={styles.topBar}>
        <View style={[styles.modePill, { borderColor: buttonColor }]}>
          ...
          <Text style={styles.modeText}>
            {mode === 'idle' ? 'Ready' : mode === 'listening' ? 'Listening' : 'Assisting'}
          </Text>
        </View>
      </View>
